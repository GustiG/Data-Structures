...
write a Python function called subarray_sum that finds the 
indices of a contiguous subarray in nums that add up to the 
target sum using a hash table (dictionary).
...
Your function should return a list of two integers 
representing the starting and ending indices of the 
subarray that adds up to the target sum. 
If there is no such subarray, your function should return 
an empty list.
...
Note that there may be multiple subarrays that add up to the 
target sum, but your function only needs to return the indices 
of any one such subarray. Also, the input list may contain both 
...
1. Function subarray_sum(nums, target)
  ...
  1.1 Initialize a dictionary called sum_index 
...
2. Loop through the nums array using enumerate to get both 
the index (i) and value (num)
  ...
  2.1 Inside the loop:
    ...
    2.1.2 Check if (current_sum - target) exists as a key in 
    the sum_index dictionary
...
3. If the loop completes and no subarray with sum equals 
to target is found, return an empty list []
'''
...
The goal of this function is to find a subarray within the 
array nums whose sum equals the target. A subarray is a 
portion of the array. The function returns the indices that 
mark the start and end of this subarray. If there is no such 
subarray, it returns an empty list [].
...
The key 0 and its corresponding value -1 are added to the 
dictionary.
...
This initial setup simplifies the for-loop code by providing 
a common starting point for summing and comparing.
...
current_sum will keep track of the sum of numbers in nums as 
...
The enumerate function helps us loop through nums and gives us 
both the index i and the value num at that index.
...
3.2 Check for Target Sum: 
if current_sum - target in sum_index: 
    - We check if the value (current_sum - target) exists as 
...
3.2.1  Return Indices
     ...
     - If the condition is true, return the start and end 
     indices of the subarray whose sum is `target`.





3.3 Update Dictionary: 
sum_index[current_sum] = i 
- If (current_sum - target) is not found in sum_index, 
add current_sum as a new key with its corresponding index i.
...
If the loop completes without finding a subarray with sum 
equals to target, return an empty list.



This approach uses a technique often referred to as 
"prefix sum" and has a time complexity of O(n), 
where n is the length of the list nums, 
as it only needs to traverse the list once. 
It's a neat and efficient solution for finding a target sum 
in a continuous subarray.

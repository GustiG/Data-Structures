...
class HashTable:
    ...
    def set_item(self, key, value):
        index = self.__hash(key)
        if self.data_map[index] == None:
            self.data_map[index] = [] # make an empty list
        self.data_map[index].append([key, value])
...
ht = HashTable()
ht.set_item("Mimi", 4)          # 1 in memory
ht.set_item("Nana", 2)          # 1 in memory
ht.set_item("Fifi", 3)          # 1 in memory
ht.set_item("Gigi", 8)          # 5 in memory
ht.print_table()
print(ht.get_item("Mimi"))
'''
################################################################
################################################################
################################################################
...
class HashTable:
    ...
    def print_table(self):
        for i, val in enumerate(self.data_map):
            print(i, ": ", val) # extra space not required

...
Your function should return a list of tuples, where each tuple 
contains two integers from arr1 and arr2 that add up to target.
...
# Expected output: [(3, 6)]
# Explanation: 
# There's only one pair that adds up to 9:
# 3 from arr1 and 6 from arr2.
...
# Expected output: []
# Explanation: 
# There are no pairs in arr1 and arr2 that add up to 11.
...
The find_pairs function takes three arguments: 
arr1, arr2, and target.

arr1 and arr2 are lists of integers.

The function creates a set called set1 containing all the 
elements in arr1.

target is also an integer, which represents the target sum 
that the function is looking for.

The function then initializes an empty list called pairs.

The function then loops through each element in arr2 and for 
each element, it calculates its complement as 
complement = target - num.

The function then checks if complement is present in set1 
using the in operator.  If it is, then the function has found 
a pair of numbers that add up to target, and it appends this 
pair (in the form of a tuple) to the pairs list.

Finally, the function returns the pairs list, which contains 
all pairs of numbers from arr1 and arr2 that add up to target.

...
using two stacks. 
Your task is to implement the enqueue method which should add 
an element to the back of the queue.
...
In order to add an element to the back of the queue, you need to 
first transfer all elements from stack1 to stack2 using a loop 
that pops each element from stack1 and pushes it onto stack2.

Once all elements have been transferred to stack2, push the new 
element onto stack1. Finally, transfer all elements from stack2 
back to stack1 in the same way as before, so that the queue 
...
class MyQueue:
    ...
    def enqueue(self, value):
        first  = self.stack1
        second = self.stack2
        while first:    # or for _ in first:
            second.append(first.pop())
        first.append(value)
        while second:   # or for _ in second:
